package co.bxvip.refresh.header;
...
import co.bxvip.refresh.OnAnimEndListener;
import co.bxvip.wedgit.BxToolIconView;
import co.bxvip.wedgit.DefaultLoadingView;
...
public class SinaRefreshView extends FrameLayout implements IHeaderView {
    ...
    private void init() {
        DefaultLoadingView rootView = new DefaultLoadingView(getContext());
        refreshArrow = rootView.getArrowView();
        refreshTextView = rootView.getTextView();
        loadingView = rootView.getLoadingView();
    ...
    public void onPullReleasing(float fraction, float maxHeadHeight, float headHeight) {
        if (fraction < 1f) {
            ...
            if (refreshArrow.getVisibility() == GONE) {
                ...
                if (loadingView instanceof BxToolIconView)
                    ((BxToolIconView) loadingView).stopProgressRun();
    ...
    public void startAnim(float maxHeadHeight, float headHeight) {
        ...
        if (loadingView instanceof BxToolIconView)
            ((BxToolIconView) loadingView).startProgressRun();
        else
