...
import co.bxvip.refresh.footer.IBottomView;
import co.bxvip.refresh.header.IHeaderView;
import co.bxvip.refresh.header.SinaRefreshView;
import co.bxvip.refresh.header.progresslayout.ProgressLayout;
import co.bxvip.refresh.processor.AnimProcessor;
import co.bxvip.refresh.processor.IDecorator;
import co.bxvip.refresh.processor.OverScrollDecorator;
import co.bxvip.refresh.processor.RefreshProcessor;
import co.bxvip.refresh.utils.DensityUtil;
 ...
 */
public class BxRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    public BxRefreshLayout(Context context) {
        this(context, null, 0);
    }

    public BxRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BxRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        mMaxHeadHeight = DensityUtil.dp2px(context, 120);
        mHeadHeight = DensityUtil.dp2px(context, 60);
        mMaxBottomHeight = DensityUtil.dp2px(context, 120);
        mBottomHeight = DensityUtil.dp2px(context, 60);
    ...
    private void addHeader() {
        ...
        extraHeadLayout.setId(IDs.refresh_ex_header);
     ...
     * 设置刷新控件监听器
     */
    private PullListener refreshListener;

    /**
     * 可是使用他的子类   RefreshListenerAdapter
     *
     * @param refreshListener
     */
    public void setOnRefreshListener(PullListener refreshListener) {
    ...
    public class CoContext {
        ...
        public void onPullingDown(float offsetY) {
            pullListener.onPullingDown(BxRefreshLayout.this, offsetY / mHeadHeight);
        }

        public void onPullingUp(float offsetY) {
            pullListener.onPullingUp(BxRefreshLayout.this, offsetY / mBottomHeight);
        }

        public void onRefresh() {
            pullListener.onRefresh(BxRefreshLayout.this);
        }

        public void onLoadMore() {
            pullListener.onLoadMore(BxRefreshLayout.this);
        }
        ...
        public void onPullDownReleasing(float offsetY) {
            pullListener.onPullDownReleasing(BxRefreshLayout.this, offsetY / mHeadHeight);
        }

        public void onPullUpReleasing(float offsetY) {
            pullListener.onPullUpReleasing(BxRefreshLayout.this, offsetY / mBottomHeight);
        }

        public boolean dispatchTouchEventSuper(MotionEvent ev) {
            return BxRefreshLayout.super.dispatchTouchEvent(ev);
        }
